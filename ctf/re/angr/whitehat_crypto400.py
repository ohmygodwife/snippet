...
def get_possible_flags():
    ...
    #  Executing a SimProcedure instead of the actual library function that gets loaded from your system makes analysis a LOT more tractable, at the cost of some potential inaccuracies, SimProcedures allow us to mitigate path explosion that would otherwise be introduced by, for example, strlen running on a symbolic string.
    # use_sim_procedures (this is a parameter to angr.Project, not cle.Loader) is True by default, extern object will be replaced with SimProcedures for dynamic linked elf, so NO need to hook for dynamic linked elf
    # this is a statically-linked binary, and it's easer for angr if we use Python
    # summaries for the libc functions
    # public start  # entry point: 0x400f4e
    # ...
    # call    sub_4018B0  #__libc_start_main
    # start endp
...
#    p.hook(0x401438, angr.SIM_PROCEDURES['stubs']['ReturnUnconstrained']())
    ...
    # string
#    @proj.hook(0x40168e, length=5)  # ONLY works for one time hook
...
#    possible_values = [ s.solver.eval_upto(s.memory.load(0x6C4B20 + i, 2), 65536, cast_to=str) for i in range(0, 8, 2) ]
    possible_values = [ s.solver.eval_upto(b, 65536, cast_to=str) for b in arg1.chop(16)]
