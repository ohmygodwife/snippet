...
def main():
    ...
    '''
    addrStrcpy = getFuncAddress('strcpy', plt=True) # could be found by objdump -d
    print hex(project.loader.main_object.plt['strcpy'])
    print project.loader.main_object.get_symbol('strcpy')
    addrBadFunc = getFuncAddress('func3') # could be found by IDA
    print hex(project.loader.find_symbol('func3').rebased_addr)
    print hex(project.loader.find_symbol('func3').linked_addr)
    print hex(project.loader.find_symbol('func3').relative_addr)
    '''
...
#    sm = sm.explore(find=check, avoid=(addrBadFunc,))
    sm = sm.explore(find=0x4006F5, avoid=(addrBadFunc,0x40070A))
